...
_I0 = 10000
_PRICE_FLOOR = 1
# P0-3 (v11): _MP recalibrated against kaggle-environments 1.32.7 MARKET_PARAMS
# (kaggriculture.py line 41-51). CARROT/TOMATO/EGG now use hinge scarcity
# below_func; the old entries (log/linear) were pre-scarcity and wrong.
# Tuple: (base, T, below_func, below_target, above_func, above_target)
_MP = {
...
# Engine hinge gain (kaggriculture.py line 58): price runs away past T.
_HINGE_GAIN = 8.0
# v10 pre-scarcity params — used ONLY when _V11_MARKET_MODEL is disabled, so
# the "A = current v10" ablation arm reproduces the old model exactly.
_MP_V10 = {
    "WHEAT": (25, 400, "sqrt", 0.80, "log", 0.20),
...
def _mshape(func, x, T=None):
    """Engine-exact shape function port (kaggriculture.py _shape, line 61-74).

    P0-3 (v11): adds the hinge branch the old model lacked. hinge degenerates
    to linear when T is missing/non-positive, matching the engine exactly.
    """
    x = max(0.0, x)
    if func == "linear":
        return x
...
def _mprice(item, inventory):
    """Engine-exact port of market_price (kaggriculture.py line 192-206).

    amp = target * base / shape(T, T)  — note the engine normalises by
    shape(f, T, T), which equals 1 for hinge at u=1 but is NOT 1 for others
    (e.g. sqrt(T)). We reproduce that exactly.
    """
    base, throughput, below_f, below_t, above_f, above_t = (
        _MP if _V11_MARKET_MODEL else _MP_V10)[item]
    if inventory < _I0:
...
def validate_market_model(verbose=True):
    """P0-3 (v11): verify _mprice() matches the engine's market_price() exactly.

    Tests every product across scarcity/glut/floor regions. Returns
    (max_abs_error, mismatches) where max_abs_error == 0 means a perfect port.
    """
    try:
        ...
        if verbose:
            print(f"[validate] 无法导入引擎 market_price: {e}")
        return (None, 0)
    test_invs = (0, 10, 100, 500, 1000, 5000, 9000, 9800, 9950,
                 10000, 10100, 10200, 10500, 11000, 12000, 15000, 20000,
                 30000, 50000)
    max_err = 0
    mismatches = 0
    for item in _MP:
        for inv in test_invs:
            try:
                eng = engine_price(item, inv)
            except Exception:
                continue
            mine = _mprice(item, inv)
            err = abs(eng - mine)
            if err > max_err:
                max_err = err
            if err != 0:
                mismatches += 1
                if verbose and mismatches <= 20:
                    print(f"  ✗ {item:>10} inv={inv:>6}: engine={eng:>4} model={mine:>4} err={err}")
    if verbose:
        print(f"[validate] max_abs_error={max_err} mismatches={mismatches}/{len(_MP)*len(test_invs)}")
    return (max_err, mismatches)
...
def _remaining_drain(item, step, shops):
    ...
    P0-2 (v11): `unlocked_shops` MAY contain duplicate instances (engine draws
    with replacement, MAX_SHOP_INSTANCES=8 caps count not variety — verified in
    kaggriculture.py line 118/890). set() would undercount demand; use Counter.
    """
    if item == "FERTILIZER":
        return 0.0  # neither the shops nor the town center consume fertilizer
    unlocked = set(shops or ())
    shop_counts = None
    if _V11_SHOP_COUNTER:
        from collections import Counter
        shop_counts = Counter(shops or ())
    live = 0
    pending = []
    for name, products in _SHOP_DEMAND.items():
        ...
        if name in unlocked:
            if shop_counts is not None:
                live += weight * shop_counts.get(name, 1)
            else:
                live += weight
        else:
...
# ===========================================================================
# v10: Opponent Observer — estimate opponent inventory/production/cash from
# public state. Key insight (community research): market inventory deltas
# decompose as Δinv = -town_drain - my_sells - opp_sells, so opp_sells can be
# recovered whenever town drain is known and my own sells are tracked.
# ===========================================================================

# Ablation switch: False = mode B (opponent observer + liquidity-aware only),
# True = mode C (adds counterfactual hold/rush timing).
_V10_COUNTERFACTUAL = True

# ===========================================================================
# v11 feature flags — each fix can be toggled independently for ablation.
# All False ⇒ behavior ≈ v10 C (rollback path).
# ===========================================================================
_V11_TRANSITION_FIX = True   # P0-1: use prev-step orders to infer prev transition
_V11_SHOP_COUNTER = True     # P0-2: Counter() instead of set() for shop instances
_V11_MARKET_MODEL = True     # P0-3: engine-exact _MP (hinge scarcity) + validate
_V11_BELIEF = True           # P1: interval/Gaussian-hybrid product belief
_V11_HAZARD = True           # P1: first-mover hazard model H=1/4/12
_V11_SEQUENTIAL = True       # P1: sequential liquidity evaluator (beam, horizon 2)
_V11_TELEMETRY = False       # debug only: print per-step market/belief diagnostics

# Per-step expected production for ONE unit of each animal/crop (engine rates):
#   COW: 3 milk / 2 days (cared) → per step = 3/48
#   SHEEP: 4 wool / 3 days (cared) → 4/72
#   GOOSE: 2 egg / 1 day (cared) → 2/24
#   WHEAT: up to 6 units / ~5 days watered
#   STRAWBERRY: 4 units / 17 days ongoing
# These are loose priors; the inventory-inference loop refines them.
_ANIMAL_RATE = {"COW": 3.0 / 48.0, "SHEEP": 4.0 / 72.0, "GOOSE": 2.0 / 24.0}
_CROP_RATE = {"WHEAT": 6.0 / 120.0, "CARROT": 4.0 / 96.0, "TOMATO": 4.0 / 288.0,
              "STRAWBERRY": 4.0 / 408.0, "MELON": 6.0 / 312.0}

_ANIMAL_UNIT = {"COW": "MILK", "SHEEP": "WOOL", "GOOSE": "EGG"}

# P1 (v11): engine-exact growth/production schedule (kaggriculture.py CROPS/ANIMALS,
# lines 11-22) used by the state-based production forecast.
_CROPS_INFO = {
    "WHEAT":      {"first": 2,  "max_day": 4,  "interval": 0, "cap": 6, "ongoing": False},
    "CARROT":     {"first": 2,  "max_day": 3,  "interval": 0, "cap": 4, "ongoing": False},
    "TOMATO":     {"first": 8,  "max_day": 8,  "interval": 1, "cap": 4, "ongoing": True},
    "STRAWBERRY": {"first": 10, "max_day": 10, "interval": 2, "cap": 4, "ongoing": True},
    "MELON":      {"first": 10, "max_day": 12, "interval": 0, "cap": 6, "ongoing": False},
}
_ANIMALS_INFO = {
...
# Town drain per step for a single shop instance on a product (weight 1).
# From _SHOP_DEMAND: shops consume 1 unit per tick (step%4==0).
_SHOP_TICK = 4


class _ProductBelief(object):
    """P1 (v11): lightweight interval/Gaussian-hybrid belief for one product.

    Replaces the old single `shed_est` scalar + `unc` counter with a belief
    that keeps a tight interval for well-identified products and automatically
    widens for MILK/WOOL-style products that hit the $1 floor (sales at $1 do
    NOT raise market inventory — engine _commit_unit line 659 — so inventory
    deltas understate opponent selling) or suffer hidden DROP losses.

    Update semantics per step:
      - no sale signal : mean += prod_rate          (production accumulates)
                         variance += drift           (uncertainty grows)
      - sale signal    : mean -= opp_sold           (confirmed outflow)
                         variance *= shrink          (signal re-converges)
      - floor price    : floor_prob rises → effective variance grows, because
                         our delta-inference becomes unreliable below floor
      - shed near cap  : drop_prob rises → widen high side (hidden losses)
    """

    __slots__ = ("item", "mean", "variance", "low", "high",
                 "prod_rate", "prod_variance", "sell_rate", "sell_variance",
                 "floor_probability", "drop_probability", "carry_probability",
                 "last_sale_step", "sales_last_4", "sales_last_12",
                 "sales_last_24", "hazard_1", "hazard_4", "hazard_12")

    def __init__(self, item, prod_rate=0.0, init_var=60.0):
        self.item = item
        self.mean = 0.0
        self.variance = init_var
        self.low = 0.0
        self.high = init_var
        self.prod_rate = prod_rate          # units/step expected production
        self.prod_variance = max(0.5, prod_rate * 0.5)
        self.sell_rate = 0.0                # units/step observed selling
        self.sell_variance = 4.0
        self.floor_probability = 0.0        # P(price at $1 → delta unreliable)
        self.drop_probability = 0.0         # P(hidden shed-overflow DROP loss)
        self.carry_probability = 0.0        # P(opponent holding in inventory)
        self.last_sale_step = -1
        self.sales_last_4 = []
        self.sales_last_12 = []
        self.sales_last_24 = []
        self.hazard_1 = 0.0                 # P(sell within next 1 step)
        self.hazard_4 = 0.0                 # P(sell within next 4 steps)
        self.hazard_12 = 0.0                # P(sell within next 12 steps)

    # -- interval maintenance ------------------------------------------------
    def _refresh_interval(self, k=2.0):
        sd = (self.variance ** 0.5)
        self.low = max(0.0, self.mean - k * sd)
        self.high = self.mean + k * sd

    def update(self, step, opp_sold, price, price_floor=1):
        """Advance belief one step. `opp_sold` is the inferred sell this step
        (0 when no sale detected); `price` is the current market price."""
        # 1) Production accumulation & drift when nothing is confirmed.
        self.mean += self.prod_rate
        self.variance += self.prod_variance + 0.4

        # 2) Sale signal handling (confirmed outflow re-converges belief).
        if opp_sold > 0:
            self.mean = max(0.0, self.mean - opp_sold)
            self.variance *= 0.55            # shrink: signal is fresh
            self.variance = max(4.0, self.variance)
            self.sell_rate = 0.7 * self.sell_rate + 0.3 * opp_sold
            self.last_sale_step = step
            for bucket, size in ((self.sales_last_4, 4),
                                 (self.sales_last_12, 12),
                                 (self.sales_last_24, 24)):
                bucket.append(opp_sold)
                if len(bucket) > size:
                    bucket.pop(0)
        else:
            self.sell_rate *= 0.96           # decay toward zero

        # 3) Floor risk: at $1, sales don't move inventory → inference blind.
        if price_floor is not None and price <= price_floor:
            self.floor_probability = min(0.95, self.floor_probability + 0.08)
            self.variance += 12.0            # widen: delta inference unreliable
        else:
            self.floor_probability *= 0.94
        # Products with tiny throughput (WOOL T=105, MILK T=122) floor often.
        # carry risk: opponent may be holding units in hand inventory.
        self.carry_probability = min(0.5, self.carry_probability + 0.005) if self.sell_rate < 0.5 else self.carry_probability * 0.9
        # DROP risk: shed is capped at 100 (engine SHED_CAP); near-cap sheds
        # silently discard overflow at end of day.
        if self.mean > 80.0:
            self.drop_probability = min(0.9, 0.5 + (self.mean - 80.0) * 0.02)
            self.variance += 8.0             # hidden losses widen high side
        else:
            self.drop_probability *= 0.93

        # 4) Cap variance (don't let a stale belief explode) and refresh.
        self.variance = min(400.0, self.variance)
        self._refresh_interval()

    def set_prod_rate(self, rate):
        self.prod_rate = rate
        self.prod_variance = max(0.5, rate * 0.5)

    # -- first-mover hazard model (P1, v11) --------------------------------
    def compute_hazards(self, step, day, hour, opp_cash, price):
        """Update hazard_1 / hazard_4 / hazard_12.

        P(opponent sells this product within H steps) via a hand-tuned
        logistic score on observable + belief features, then
            P(within H) = 1 - prod_{t=1..H}(1 - hazard_t)
        using a constant per-step hazard approximation.

        Features:
          time_since_last_sale, sales_last_4/12/24, avg_sale_qty,
          sale_burstiness, belief mean (hold pressure), production rate,
          opponent cash, day/hour, current price, distance-to-floor.
        """
        # ---- score features ----
        since = (step - self.last_sale_step) if self.last_sale_step >= 0 else 999
        s4 = sum(self.sales_last_4) / float(max(1, len(self.sales_last_4)))
        s12 = sum(self.sales_last_12) / float(max(1, len(self.sales_last_12)))
        s24 = sum(self.sales_last_24) / float(max(1, len(self.sales_last_24)))
        burst = (s4 / max(0.1, s24)) if s24 > 0 else 1.0
        hold_pressure = min(1.0, self.mean / 80.0)
        prod_rate = self.prod_rate
        floor_risk = max(0.0, 1.0 - price / float(max(1, _MP[self.item][0])))

        # ---- hand-tuned logistic score ----
        z = -3.0                                          # baseline log-odds
        z += 1.8 * min(1.0, self.sell_rate / 2.0)         # observed sell rate
        z += 1.2 * min(1.0, s4 / 15.0)                    # recent 4-step avg qty
        z += 0.6 * min(1.0, burst)                        # burstiness (clumping)
        z += 1.5 * hold_pressure                          # holding pressure
        z += 0.8 * min(1.0, prod_rate * 24.0 / 3.0)       # production pressure
        if since <= 6:
            z += 0.7                                      # recent seller, continues
        elif since >= 48:
            z += 0.5                                      # long silence → dump
        if day >= 26:
            z += 1.2                                      # liquidation window
        elif day >= 24:
            z += 0.6
        if opp_cash > 8000 and day >= 26:
            z += 0.8                                      # cash-rich late → dumping
        z += 0.9 * floor_risk                             # near floor → race to sell
        if hour >= 20:
            z += 0.3                                      # late-day flush

        # ---- per-step hazard → horizon probabilities ----
        h1 = 1.0 / (1.0 + _math.exp(-z))
        h1 = min(0.85, max(0.01, h1))
        self.hazard_1 = h1
        self.hazard_4 = 1.0 - (1.0 - h1) ** 4
        self.hazard_12 = 1.0 - (1.0 - h1) ** 12
        return self.hazard_1, self.hazard_4, self.hazard_12

    def p_sell_within(self, horizon, recent=None):
        """P(opponent sells this product within `horizon` steps).

        Lightweight hazard: baseline from observed sell rate, boosted by how
        long it's been since the last sale (time-since-last-sale effect) and
        by how much the opponent is holding. Saturates at 1.
        """
        if horizon <= 0:
            return 0.0
        rate = self.sell_rate if self.sell_rate > 0 else 0.0
        # time-since-last-sale: if they sold recently, another sale is more
        # likely soon (batching); if it's been a long time, they're hoarding.
        since = 0
        if self.last_sale_step >= 0:
            since = max(0, (recent or 0) - self.last_sale_step)
        base = rate * 0.5
        if since <= 6:
            base += 0.10                    # recent seller, likely continues
        elif since >= 48:
            base += 0.06                    # long silence → big dump coming
        hold = min(1.0, self.mean / 80.0) * 0.20
        p = base + hold
        p *= horizon
        return min(1.0, p)


class _OppState(object):
    """Per-game tracker of the opponent's public & inferred state."""
    ...
    def __init__(self):
        ...
        self.plants = {}    # crop -> count
        self.shed_est = {}  # product -> estimated shed inventory (raw units)
        self.unc = {}       # product -> uncertainty (absolute units)
        self.sell_hist = {}  # product -> deque of recent inferred sells
        self.prod_est = {}  # product -> per-step production estimate
        self.beliefs = {}   # P1 (v11): item -> _ProductBelief (when enabled)
        self.mirror = False
        self.opp_liquidating = False
        self.step = -1
        # --- P0-1 transition bookkeeping (v11) ---
        # The engine executes MY orders at step N, then the next observation
        # (step N+1) reflects that execution. So `cur` obs inventory is the
        # result of the PREVIOUS step's orders. We therefore snapshot prev
        # state and infer the prev transition, never mixing current orders.
        self.prev_market_inv = {}  # inventory as seen in the previous obs
        self.prev_my_orders = []   # my market orders issued at the prev step
        self.prev_step = -1        # prev step counter (guard: +1 == current)
        self._inited = False       # True once we have a previous snapshot
        # P0-1 diagnostic counters (telemetry)
        self.infers_done = 0
        self.infers_skipped = 0

    def observe(self, obs, my_sells):
        """Called every step with full obs and my own sell orders this step."""
        farms = obs.get("farms") or []
        if len(farms) < 2:
            return
        player = int(obs.get("player", 0) or 0)
        ...
        self.step = step
        self.cash = float(opp.get("money", 0) or 0)
        self.hands = len(opp.get("hands") or [])
        self.quads = len(opp.get("unlocked_quadrants") or [])

        # Public farm structure (animals & plants are visible on tiles).
        # P1 (v11): keep detailed tile state (planted_day, yield_units,
        # fertilized_until_day, placed_day) so production can be FORECAST
        # from actual growth stage, not just counted with an average rate.
        animals, plants = {}, {}
        tile_plants, tile_animals = [], []
        ...
        self.plants = plants
        self.tile_plants = tile_plants
        self.tile_animals = tile_animals

        # Production estimate: state-based forecast (v11) with average-rate
        # prior as fallback. Forecast horizon = 1 day (24 steps): how much
        # this product will be produced over the next day, converted to a
        # per-step rate.
        if _V11_BELIEF:
            prod = {}
            day = step // 24
            # Track which products have ANY visible driver tile. Fallback
            # prior may only fire for products with ZERO visible drivers —
            # a visible-but-immature cow must not be overridden to "producing".
            seen_units = set()
            # -- animals: first_yield_day + interval schedule (engine ANIMALS)
            for ta in tile_animals:
                a = ta["animal"]
                info = _ANIMALS_INFO.get(a)
                unit = _ANIMAL_UNIT.get(a)
                if not info or not unit:
                    continue
                seen_units.add(unit)
                age_days = day - ta["placed_day"]
                if age_days < info["first"]:
                    continue  # not producing yet
                # expected units in next 24h: 1 production tick at interval
                # (cared animals produce more; treat fed+cared as 1.5x)
                per_tick = info["cap"] if ta.get("fed_today") else info["cap"] * 0.5
                if ta.get("cared_today"):
                    per_tick *= 1.5
                prod[unit] = prod.get(unit, 0.0) + per_tick / 48.0  # per step
            # -- crops: remaining yield over window / harvest schedule
            for tp in tile_plants:
                c = tp["crop"]
                info = _CROPS_INFO.get(c)
                if not info:
                    continue
                seen_units.add(c)
                age_days = day - tp["planted_day"]
                if info["ongoing"]:
                    # produces every interval days after first_yield_day
                    if age_days >= info["first"]:
                        prod[c] = prod.get(c, 0.0) + info["cap"] / (info["interval"] * 24.0)
                else:
                    # one-time: window_start..max_yield_day watering grows yield;
                    # if still inside window, expect the remaining growth soon
                    window_start = (info["max_day"] + 1) // 2
                    if age_days >= info["first"] and age_days <= info["max_day"]:
                        # harvested crop will enter market within ~1 day
                        prod[c] = prod.get(c, 0.0) + max(1.0, tp["yield_units"]) / 24.0
            self.prod_est = prod
            # Fallback prior ONLY for products with zero visible driver tiles.
            # A visible-but-immature cow produces nothing — never override.
            for a, n in animals.items():
                rate = _ANIMAL_RATE.get(a, 0.0)
                unit = _ANIMAL_UNIT.get(a)
                if rate and unit and unit not in seen_units:
                    self.prod_est[unit] = n * rate * 0.5
            for c, n in plants.items():
                rate = _CROP_RATE.get(c, 0.0)
                if rate and c not in seen_units:
                    self.prod_est[c] = self.prod_est.get(c, 0.0) + n * rate * 0.5
        else:
            # v10 fallback: plain count * average rate
            prod = {}
            for a, n in animals.items():
                rate = _ANIMAL_RATE.get(a, 0.0)
                unit = _ANIMAL_UNIT.get(a)
                if rate and unit:
                    prod[unit] = prod.get(unit, 0.0) + n * rate
            for c, n in plants.items():
                rate = _CROP_RATE.get(c, 0.0)
                if rate:
                    prod[c] = prod.get(c, 0.0) + n * rate
            self.prod_est = prod

        # ---------------------------------------------------------------
        # P0-1 (v11): transition inference uses the PREVIOUS snapshot only.
        # Engine timeline (verified in kaggriculture.py interpreter):
        #   step N: agent acts → engine executes market orders → town consumes
        #   step N+1 obs: reflects step N's execution.
        # So the market delta between `prev_market_inv` and `cur` was caused
        # by orders issued at `prev_step` — NOT by the current `my_sells`.
        # We infer opp_sells for that PREV transition, then snapshot current.
        # ---------------------------------------------------------------
        # P0-1 (v11): transition inference uses the PREVIOUS snapshot only.
        # Engine timeline (verified in kaggriculture.py interpreter):
        #   step N: agent acts → engine executes market orders → town consumes
        ...
        prev_ok = (self._inited and self.prev_step + 1 == step)
        if _V11_TRANSITION_FIX:
            if prev_ok:
                self.infers_done += 1
                my_sold_prev = {}
                for order in self.prev_my_orders or []:
                    if (isinstance(order, list) and len(order) >= 3
                            and order[0] == "SELL"):
                        item = order[1]
                        my_sold_prev[item] = (my_sold_prev.get(item, 0)
                                              + max(0, int(order[2] or 0)))
                for item in _MP:
                    if item == "FERTILIZER":
                        continue
                    cur = int(market_inv.get(item, _I0) or _I0)
                    prev = int(self.prev_market_inv.get(item, _I0) or _I0)
                    delta = cur - prev
                    drain = _per_step_drain(item, self.prev_step, shops)
                    mine = my_sold_prev.get(item, 0)
                    opp_sold = -delta - drain - mine
                    opp_sold = max(0.0, opp_sold)
                    if opp_sold > 0:
                        hist = self.sell_hist.setdefault(item, [])
                        hist.append(opp_sold)
                        if len(hist) > 24:
                            hist.pop(0)
                        self.shed_est[item] = max(0.0, self.shed_est.get(item, 0.0)
                                                  + self.prod_est.get(item, 0.0) * 1.0 - opp_sold)
                        self.unc[item] = min(400.0, self.unc.get(item, 0.0) + 2.0)
                    else:
                        self.shed_est[item] = max(0.0, self.shed_est.get(item, 0.0)
                                                  + self.prod_est.get(item, 0.0))
                        self.unc[item] = min(400.0, self.unc.get(item, 0.0) + 1.0)
                    if opp_sold > 0:
                        self.unc[item] = max(0.0, self.unc[item] - 6.0)
                    # P1 (v11): belief update on the same inferred transition.
                    if _V11_BELIEF:
                        bel = self.beliefs.get(item)
                        if bel is None:
                            bel = _ProductBelief(item,
                                                 prod_rate=self.prod_est.get(item, 0.0))
                            self.beliefs[item] = bel
                        bel.update(self.prev_step, opp_sold,
                                   int(prices_now.get(item, _mprice(item, cur)) or 0))
                        if _V11_HAZARD:
                            bel.compute_hazards(
                                self.prev_step, self.prev_step // 24,
                                self.prev_step % 24, self.cash,
                                int(prices_now.get(item, _mprice(item, cur)) or 0))
            else:
                self.infers_skipped += 1
                # First call or step gap: no reliable transition; anchor only.
                for item in _MP:
                    if item != "FERTILIZER":
                        cur = int(market_inv.get(item, _I0) or _I0)
                        if not self._inited:
                            self.shed_est[item] = 0.0
                            self.unc[item] = 60.0
                            if _V11_BELIEF:
                                self.beliefs[item] = _ProductBelief(
                                    item, prod_rate=self.prod_est.get(item, 0.0))
                        self.prev_market_inv[item] = cur
        else:
            # ---- v10 fallback path (rollback): old single-step inference ----
            my_sold = {}
            for order in my_sells or []:
                if isinstance(order, list) and len(order) >= 3 and order[0] == "SELL":
                    item = order[1]
                    my_sold[item] = my_sold.get(item, 0) + max(0, int(order[2] or 0))
            for item in _MP:
                if item == "FERTILIZER":
                    continue
                cur = int(market_inv.get(item, _I0) or _I0)
                ...
                self.prev_market_inv[item] = cur
                delta = cur - prev
                drain = _per_step_drain(item, step, shops)
                mine = my_sold.get(item, 0)
                opp_sold = -delta - drain - mine
                opp_sold = max(0.0, opp_sold)
                if opp_sold > 0:
                    hist = self.sell_hist.setdefault(item, [])
                    ...
                    if len(hist) > 24:
                        hist.pop(0)
                    cur_est = self.shed_est.get(item, 0.0) + self.prod_est.get(item, 0.0) * 1.0 - opp_sold
                    self.shed_est[item] = max(0.0, cur_est)
                    self.unc[item] = min(400.0, self.unc.get(item, 0.0) + 2.0)
                else:
                    self.shed_est[item] = max(0.0, self.shed_est.get(item, 0.0)
                                              + self.prod_est.get(item, 0.0))
        ...
        # Snapshot current state for the NEXT step's transition inference.
        if _V11_TRANSITION_FIX:
            for item in _MP:
                if item != "FERTILIZER":
                    self.prev_market_inv[item] = int(market_inv.get(item, _I0) or _I0)
            self.prev_my_orders = list(my_sells)
            self.prev_step = step
            self._inited = True

        # Liquidation heuristic: late season + high cash + big sheds → dumping.
        day = step // 24
        self.opp_liquidating = day >= 26 and self.cash > 8000

    def likely_sell_soon(self, item, threshold):
        """P(item sells a meaningful chunk soon), from recent sell rate."""
        hist = self.sell_hist.get(item) or []
        if not hist:
            return 0.0
        recent = sum(hist[-6:]) / float(max(1, len(hist[-6:])))
        return min(1.0, recent / max(1.0, threshold))

    def expected_opponent_market_sell(self, item, horizon, price=None):
        """P1 (v11): expected units of `item` the opponent sells into the
        shared market within `horizon` steps.

        This replaces the old `opp_prod * horizon` proxy: production is NOT
        market supply. The expectation combines:
          - belief shed mean (they can only sell what they hold),
          - sell hazard within horizon (P they sell at all),
          - recent observed sale size (how much a typical dump is),
          - production inflow over the horizon (replenishes the shed),
          - late-season liquidation forcing.
        """
        if horizon <= 0:
            return 0.0
        bel = self.beliefs.get(item) if _V11_BELIEF else None
        held = 0.0
        if bel is not None:
            held = bel.mean
        else:
            held = self.shed_est.get(item, 0.0)
        # P(they sell within horizon): from hazard when available.
        p_sell = 0.0
        if bel is not None and _V11_HAZARD:
            if horizon <= 1:
                p_sell = bel.hazard_1
            elif horizon <= 4:
                p_sell = bel.hazard_4
            else:
                p_sell = bel.hazard_12
        else:
            p_sell = min(1.0, self.likely_sell_soon(item, 15.0) * 2.0)
        # Typical dump size from recent inferred sells.
        hist = self.sell_hist.get(item) or []
        typical = sum(hist[-6:]) / float(max(1, len(hist[-6:]))) if hist else 0.0
        typical = max(typical, 5.0)
        # Production inflow replenishes the shed during the horizon.
        inflow = self.prod_est.get(item, 0.0) * horizon
        available = held + inflow
        expected = min(available, p_sell * typical * (horizon / 4.0))
        # Late-season: liquidation accelerates.
        if self.opp_liquidating:
            expected *= 1.5
        return max(0.0, expected)


def _per_step_drain(item, step, shops):
    """Expected town consumption of `item` in the step AFTER `step`."""
    # Shops tick every 4 steps; town center every 12. Only count the next tick.
    # P0-2 (v11): use Counter — unlocked_shops may hold duplicate instances
    # (engine draws with replacement; verified kaggriculture.py line 118/890).
    drain = 0.0
    nxt = step + 1
    if nxt % 4 == 0:
        unlocked = set(shops or ())
        ...
        for name, products in _SHOP_DEMAND.items():
            if item in products:
                if name in unlocked:
                    if shop_counts is not None:
                        live += (2 if len(products) == 1 else 1) * shop_counts.get(name, 1)
                    else:
                        live += 2 if len(products) == 1 else 1
                elif len(_SHOP_DEMAND) - len(unlocked) > 0:
                    live += (2 if len(products) == 1 else 1) * 0.5  # expected unlock
        drain += live
    if nxt % 12 == 0 and item in _CENTER_ITEMS:
        day = nxt // 24
        drain += 4 if day >= 20 else (2 if day >= 10 else 1)
    return drain
...
def _opp_est(item):
    """Convenience accessor: (estimate, uncertainty) or (0, 0)."""
    return _OPP.shed_est.get(item, 0.0), _OPP.unc.get(item, 0.0)


def _telemetry(obs, step, sells, keep, short, money):
    """P1 (v11): local debug dump explaining WHY each sell was chosen.
    Fields: step, item, market_inv, inferred_opp_sell, belief mean/low/high,
    floor_probability, P_first_sell (h1/h4), EV_now vs EV_wait, liquidity,
    final score. Gated behind _V11_TELEMETRY (False on live submission)."""
    inventory = ((obs.get("market") or {}).get("inventory") or {})
    prices = ((obs.get("market") or {}).get("prices") or {})
    shops = (obs.get("town") or {}).get("unlocked_shops") or []
    player = int(obs.get("player", 0) or 0)
    farm = ((obs.get("farms") or [{}])[player])
    all_sells = list(sells) + [o for o in (keep or [])
                               if isinstance(o, list) and o and o[0] == "SELL"]
    print(f"[telemetry] step={step} day={step//24} hour={step%24} "
          f"cash={money:.0f} short={short:.0f}", flush=True)
    for order in all_sells:
        ...
        ev_now = unit * qty
        drain = _per_step_drain(item, step, shops)
        steps_ahead = max(1, 4 - (step % 4))
        opp_sell = _OPP.expected_opponent_market_sell(item, steps_ahead) if (_V11_BELIEF and _V11_HAZARD) else 0.0
        fut_inv = max(1, inv - drain * steps_ahead - opp_sell)
        ev_wait = _mprice(item, fut_inv) * qty
        bel = _OPP.beliefs.get(item) if _V11_BELIEF else None
        b_mean = b_low = b_high = fprob = p1 = p4 = 0.0
        if bel is not None:
            b_mean, b_low, b_high = bel.mean, bel.low, bel.high
            fprob = bel.floor_probability
            p1, p4 = bel.hazard_1, bel.hazard_4
        opp_sold_recent = sum((_OPP.sell_hist.get(item) or [])[-4:]) / 4.0
        print(f"  sell {item:>10} qty={qty:>3} inv={inv:>5} price={unit:>3} "
              f"EV_now={ev_now:>7.0f} EV_wait={ev_wait:>7.0f} "
              f"opp_sold_est={opp_sell:>5.1f} opp_recent={opp_sold_recent:>5.1f} "
              f"bel={b_mean:.0f}[{b_low:.0f},{b_high:.0f}] floor_p={fprob:.2f} "
              f"P_sell1={p1:.2f} P_sell4={p4:.2f}", flush=True)


def _v11_beam_order(orders, obs, money, short):
    """P1 (v11): sequential liquidity evaluator over SELL order sequences.

    Market queue semantics (engine _process_market): orders resolve in list
    order, and an earlier SELL funds a later BUY in the SAME turn. So the
    value of selling item A first is not just A's revenue — it also
    determines whether the farm can afford the buy orders that follow.

    We run a tiny beam search (horizon 2, width 4) over the candidate SELL
    orders. Each node tracks the running cash after executing its prefix;
    each step's utility is
        revenue − price_damage + liquidity_value(shortfall now funded)
    and the sequence score is the sum. The returned order maximizes
    sequential value while staying O(beam²) — cheap enough per turn.

    Returns the reordered order list (only orders present in `orders`).
    """
    if len(orders) <= 1:
        return list(orders)
    inventory = ((obs.get("market") or {}).get("inventory") or {})
    ...
    def step_utility(order, cash):
        """Utility of executing `order` when running cash = cash."""
        item = order[1]
        try:
            qty = int(order[2] or 0)
        except (TypeError, ValueError):
        ...
        inv = int(inventory.get(item, _I0) or _I0)
        unit = _mprice(item, inv)
        revenue = float(unit) * qty
        price_damage = float(qty) * float(unit - _mprice(item, inv + qty))
        # Liquidity: how much of the remaining shortfall this sale funds.
        liq = min(revenue, max(0.0, shortfall - cash)) * 0.35
        return revenue - price_damage + liq, revenue

    # Beam: list of (score, cash, sequence)
    beam = [(0.0, money, [])]
    used = set()
    for depth in range(min(2, len(orders))):
        nxt = []
        for score, cash, seq in beam:
            for i, order in enumerate(orders):
                if i in used and False:
                    continue
                if any(o is order for o in seq):
                    continue
                u, rev = step_utility(order, cash)
                nxt.append((score + u, cash + rev, seq + [order]))
        nxt.sort(key=lambda x: -x[0])
        beam = nxt[:4]
    if not beam:
        return list(orders)
    best = max(beam, key=lambda x: x[0])
    # Append any orders the beam did not include (keeps all sells).
    chosen = list(best[2])
    for o in orders:
        if o not in chosen:
            chosen.append(o)
...
def agent(obs, config=None):
    ...
    try:
        ...
        orders = list(action.get("market") or [])
        # v10: observe opponent BEFORE planning sells, using this step's own
        # sell orders to decompose the market inventory delta.
        try:
            _OPP.observe(obs, orders)
        except Exception:
            pass
        keep = [
        ...
        # P1 (v11): local debug telemetry — explains WHY a sell was chosen.
        # Never enabled on the live submission (_V11_TELEMETRY defaults False).
        if _V11_TELEMETRY:
            try:
                _telemetry(obs, step, sells, keep, short, money)
            except Exception:
                pass
        if not _SORT_SELLS:
        ...
        # v10: liquidity-aware ranking. Rank sells by
        #   Utility = Revenue + λ·CashUnlock − μ·PriceDamage + ρ·OpponentDamage
        # where CashUnlock is how much of this turn's buy budget the sale funds
        # (selling a big-ticket item first keeps the farm operational — this is
        # the actual mechanism behind Cleo's liquidity dominance), and
        # OpponentDamage prefers selling products the opponent also holds
        # heavily (beat them to the price before they glut it).
        def _v10_priority(order):
            ...
            unit = _mprice(item, inv)
            revenue = float(unit) * qty
            # PriceDamage = revenue lost to walking the price down ourselves.
            price_damage = float(qty) * float(unit - _mprice(item, inv + qty))
            # CashUnlock: if cash is short of what this turn's buys need, a
            # sale that funds the shortfall is worth extra (λ scales with need).
            cash_unlock = 0.0
            if short > 0 and money < 50.0:
                lambda_c = 0.35
                cash_unlock = lambda_c * min(revenue, short)
            # OpponentDamage: opponent's estimated shed of this product.
            opp_est, opp_unc = _opp_est(item)
            opp_factor = 0.0
            if opp_est > 20.0 and opp_unc < 200.0:
                rho = 0.12
                opp_factor = rho * min(1.0, opp_est / 150.0) * revenue
            # v10/v11 counterfactual timing: compare selling NOW vs waiting for
            # the next town consumption tick (shops drain every 4 steps).
            # If town demand will lift the price (scarcity) and the opponent is
            # NOT about to glut it, holding is worth bonus — priority goes DOWN.
            # If the opponent is about to dump the same product, waiting is a
            # trap — priority goes UP (sell before they crash the price).
            #
            # P1 (v11): the expected opponent market supply is now estimated
            # from belief + hazard (expected_opponent_market_sell) — NOT from
            # `opp_prod * horizon` (production ≠ market supply).
            hold_bonus = 0.0
            rush_bonus = 0.0
            if _V10_COUNTERFACTUAL and step < _RAMP_START:  # investment phase
                steps_ahead = 4 - (step % 4)
                if steps_ahead <= 0:
                    steps_ahead = 4
                drain_rate = _per_step_drain(item, step, shops)
                if _V11_BELIEF and _V11_HAZARD:
                    opp_sell_ahead = _OPP.expected_opponent_market_sell(
                        item, steps_ahead)
                    bel = _OPP.beliefs.get(item)
                    p_first = bel.hazard_4 if bel is not None else 0.0
                else:
                    # v10 fallback path (rollback): old opp_prod proxy.
                    opp_sell_ahead = _OPP.prod_est.get(item, 0.0) * steps_ahead
                    p_first = _OPP.likely_sell_soon(item, 15.0)
                # Projected inventory after the next shop tick.
                future_inv = max(1, inv - drain_rate * steps_ahead
                                 - opp_sell_ahead)
                price_later = _mprice(item, future_inv)
                price_gain = (price_later - unit) * qty
                if price_gain > 0:
                    # Town demand will lift price and opponent isn't dumping.
                    hold_bonus = 0.9 * min(price_gain, 0.25 * revenue)
                if p_first > 0.55 and opp_sell_ahead > 0.01:
                    # Opponent actively selling this product — waiting loses.
                    rush_bonus = 0.7 * p_first * min(revenue, 4000.0)
            return (revenue - 1.0 * price_damage + cash_unlock + opp_factor
                    - hold_bonus + rush_bonus)

        merged = [o for o in sells if promotable(o)] + [o for o in keep if promotable(o)]
        if _V11_SEQUENTIAL and len(merged) > 1:
            # P1 (v11): sequential liquidity evaluator — order the promotable
            # sells by beam-search over SELL sequences (liquidity-aware),
            # instead of the single-step priority ranking.
            merged = _v11_beam_order(merged, obs, money, short)
        else:
            merged.sort(key=lambda o: -_v10_priority(o))
