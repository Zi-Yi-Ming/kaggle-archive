...
吃不下空间结构**。像素不是独立的 784 个特征,它们是 28×28 的二维图像,
相邻像素有空间关系,全连接把这种关系打散了。这促使我下定决心学 CNN。
...
baseline RF/PCA+LR/MLP 三模型对比 (0.96378)
 │  RF 最优但摸不到 0.99;MLP 0.954 垫底 → 证明全连接吃不下空间结构
 │  ↓ 范式切换:表格思维 → 图像思维
...
**范式切换的动机**:MLP 0.954 垫底是决定性信号——不是参数问题,是架构问题。
全连接对图像是"错的问题形式",卷积才是正解。
